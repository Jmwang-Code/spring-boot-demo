...
## 4.1 Hadoop CAP原则
1. HDFS的副本机制，HDFS将数据分成多个块、并在不同节点存储，某个节点挂掉，HDFS也可以从其他节点上恢复数据。
2. YARN的资源调度机制，划分多个容器，并在不同节点上运行多个容器，某个节点挂掉，YARN也可以从其他节点上恢复容器。

保证了Hadoop的高可用性和分区容错性

3. zookeeper的分布式(锁)协调机制，保证了Hadoop的一致性。