...
//            }
        ...
        //计算对象内存
        函数DeepTest.printMemoryUsage(Runtime.getRuntime());
    ...
    @Test
    public void testDeepWithLargeData() {
        assertEquals(15, trie.deep());
        trie.add("newWord".codePoints().toArray(), 1000001, 1);
        assertEquals(15, trie.deep());
    }

    @Test
    public void testDeepWithSpecificStructure() {
        trie.add("prefix".codePoints().toArray(), 1000002, 1);
        assertEquals(15, trie.deep());
    }

    @Test
    public void testDeepAfterAddingAndRemovingElements() {
        trie.add("word".codePoints().toArray(), 1000003, 1);
        trie.remove("word".codePoints().toArray(), 1000003, 1);
        assertEquals(15, trie.deep());
    }
}
