...
public class LCR090打家劫舍II {
    ...
    public int rob3(int[] nums) {
        ...
        int[][] dp = new int[nums.length + 1][nums.length + 1];
        //当end为nums.length
        dp[nums.length][nums.length] = 0;
        dp[nums.length-1][nums.length] = nums[nums.length - 1];
        dp[nums.length-2][nums.length] = nums[nums.length - 2];
        //当end为nums.length-1
        dp[nums.length-1][nums.length-1] = nums[nums.length - 1];
        dp[nums.length-2][nums.length-1] = nums[nums.length - 2];
        dp[nums.length-3][nums.length-1] = nums[nums.length - 3];

        //计算的时候分为 end 范围为 [0,nums.length-1] 和 [1,nums.length] 两种情况
        for (int i = nums.length-3; i > 0; i--) {
            dp[i][nums.length] = Math.max(nums[i] + dp[i + 2][nums.length],nums[i] + dp[i + 3][nums.length]);
        }

        for (int i = nums.length-4; i >= 0; i--) {
            if (nums.length <= 4){
                dp[i][nums.length-1] = Math.max(nums[i] + dp[i + 2][nums.length-1],nums[i]);
            }else {
                dp[i][nums.length-1] = Math.max(nums[i] + dp[i + 2][nums.length-1],nums[i] + dp[i + 3][nums.length-1]);
            }
        }


        return Math.max(dp[0][nums.length-1],Math.max(dp[1][nums.length],dp[2][nums.length]));
    }
